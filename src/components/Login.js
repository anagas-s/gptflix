...
const Login = () => {
  ...
  .then((userCredential) => {
    ...
    updateProfile(user, {
        ...
        photoURL: "https://avatars.githubusercontent.com/u/115894733?v=4",
      }).then(() => {
        // Profile updated!
  ...
  .catch((error) => {
    ...
    return (
        ...
        <form onSubmit={(e)=>e.preventDefault()} className="absolute bg-opacity-80 p-12 text-white bg-black w-3/12 my-36 mx-auto right-0 left-0 rounded-lg">
           ...
           {!isSignInForm && <input ref={name} type="text" placeholder="Name" className="p-3 my-4 w-full bg-gray-600" />}
